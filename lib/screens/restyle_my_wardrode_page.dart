...
class _RestyleMyWardrobePageState extends State<RestyleMyWardrobePage> {
  ...
  List<String> _generateOutfitCombinations(List<ClothingItem> closet) {
    ...
    List<ClothingItem> tops = closet.where((item) => item.baseType == 'Top').toList();
    List<ClothingItem> bottoms = closet.where((item) => item.baseType == 'Bottom').toList();
    List<ClothingItem> dresses = closet.where((item) => item.baseType == 'Dress').toList();
    List<ClothingItem> outerwears = closet.where((item) => item.baseType == 'Outerwear').toList();
    ...
    for (var dress in dresses) {
      ...
      if (footwear.isNotEmpty) {
        suggestion += ' with ${footwear.first.color} Footwear (${footwear.first.footwearKind ?? 'kind not specified'})';
      }
      if (accessories.isNotEmpty) {
        suggestion += ' and a ${accessories.first.color} ${accessories.first.specificType ?? 'Accessory'}';
      }
    ...
    for (var top in tops) {
      for (var bottom in bottoms) {
        for (var shoe in footwear) {
          ...
          if (accessories.isNotEmpty) {
            suggestion += ' with a ${accessories.first.color} ${accessories.first.specificType ?? 'Accessory'}';
          }
          suggestions.add(suggestion);
        }
          ...
          children: [
            ...
            if (_selectedType == 'Footwear')
              TextField(
                ...
                decoration: const InputDecoration(labelText: 'Kind of Footwear (e.g., sneakers, heels)'),
              ),
